...
const int NUM_PARTICLES = 1024*1024*2;		//global work size
const int LOCAL_SIZE    = 64;				//local work-group size
const char *CL_FILE_NAME   = { "particles.cl" };
const char *CL_BINARY_NAME = { "particles.nv" };


const int GLUITRUE  = { true  };
const int GLUIFALSE = { true };

#define ESCAPE		0x1b

const int INIT_WINDOW_SIZE = { 720 };		// window size in pixels
...
void	PrintOpenclInfo();		//aaaaa
void	Quit( );
float	Ranf( float, float );
void	Reset( );
void	ResetParticles( );
void	Resize( int, int );
void	SelectOpenclDevice();	//aaaaa
void	Traces( int );
void	Visibility( int );
...
InitLists( )
{
	...
	// ****************************************
		...
		glColor3f(0., .9f, .9f);	
		glPushMatrix();
		glTranslatef(750.f, -500.f, 0.f);	// sphere center
		glutWireSphere(600.f, 100, 100);	// sphere radius, slices, stacks
		glPopMatrix();
